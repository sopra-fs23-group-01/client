...
const ProfilePage=() =>{
  ...
  //Used to transform to the normal uniform dd-mm-yyyy
  const formatDate = (dateString) => {
    if (dateString!=null){
    const newStr1 = dateString.substring(0, dateString.indexOf(' '));
    const date = new Date(newStr1);
    const newStr = `${date.getDate().toString().padStart(2, '0')}-${(date.getMonth() + 1).toString().padStart(2, '0')}-${date.getFullYear()}`;
    return newStr;};
  ...
  function Profilefield({functionuser}) {
    ...
    return (
      <div className="profile container">
        ...
        <div className="profile name">Creation date:  {formatDate(functionuser.registerDate)}</div>
        <div className="profile name">Birthday:    {formatDate(functionuser.birthday)}
                        </div>
