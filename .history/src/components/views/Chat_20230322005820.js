    ...
    useEffect(() => {
        async function start() {
            if (!localStorage.getItem('username')) {
                await axios.get("http://sopra-fs23-group-01-server.oa.r.appspot.com:8080/getName").then(response => {
                    localStorage.setItem('username', response.data)
                })
            }
            username = localStorage.getItem('username')
            let baseUrl = "ws://sopra-fs23-group-01-server.oa.r.appspot.com:8080/websocket/"
