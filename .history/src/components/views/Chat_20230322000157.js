    ...
    useEffect(() => {
        async function start() {
            if (!localStorage.getItem('username')) {
                await axios.get("http://sopra-fs23-liu-zihan-client.oa.r.appspot.com/getName").then(response => {
                    localStorage.setItem('username', response.data)
                })
            }
            username = localStorage.getItem('username')
            let baseUrl = "ws://sopra-fs23-liu-zihan-client.oa.r.appspot.com/websocket/"
